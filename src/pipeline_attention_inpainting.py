...
class StableDiffusionAttentionStoreInpaintPipeline(
    ...
    # function to register the attention control in the unet
    def register_attention_control(self):
        ...
        for name in self.unet.attn_processors.keys():
            ...
            cross_att_count += 1
            attn_procs[name] = AttnProcessorStore(attnstore=self.attention_store, place_in_unet=place_in_unet)
    ...
    def save_plot_attention(self, prompt, out_name='foo.png'):
        ...
        plt.figure(figsize=(1.5*n_timesteps,1.5*n_indx))
        i=1

        for idx in index_to_plot:
            first=True
            for step in average_attention.keys():
                plt.subplot(n_indx, n_timesteps, i)
                if first:
                    plt.text(-10, 25, prompt_index[idx].replace('</w>', ''), fontsize=12, ha='center', rotation=90)
                    first=False
                attention_array = average_attention[step][:,:,idx].detach().cpu().numpy()
                plt.imshow(attention_array)
                i+=1
                plt.axis('off')
        plt.tight_layout()
...
def generate_random_sum(n):
    ...
    random_list = [random.uniform(0, 1) for _ in range(n - 1)]
    random_list.sort()
    random_list = [0] + random_list + [1]
    result = [random_list[i + 1] - random_list[i] for i in range(n)]
    
    return result

class AttnProcessorStore:
...
# class to store the attention maps
class AttentionStore:
    ...
    def between_steps(self):
        self.attention_store[self.curr_step_index] = self.step_store
    ...
    def reset(self):
        print('resetting attention store')
        self.cur_att_layer = 0
        self.step_store = self.get_empty_store()
...
# TODO - this is the new version of the attention processor, has to be tested and integrated
""" 
class AttnProcessor2_0_with_store:
    ...
    def __call__(
        ...
        print('hidden states reshape', hidden_states.shape)
        self.attnstore(hidden_states, is_cross, self.place_in_unet)
